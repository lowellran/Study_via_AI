...
| **节奏适应力** | 毫无模式区分，一刀切长文 | 机械固定大纲，难以瘦身 | **原生双轨自适应**：⚡ `/quick` 1~3节点15~45min敏捷闭环直奔点灯/落地；🔥 `deep` 20h 10-Plan 深度大系，支持无损升级 |
...
* `/mode <quick|deep>`：切换模式（`quick` 极速突击：1~3 敏捷节点 15~45min 靶向精讲 + 1 题工业雷区速测；`deep` 深度攻坚：5 级阶梯 20h 10次计划 + 双重考核 + 全量落盘，支持随时无损互转）。
* `/quick`：极速启动前缀（如 `/quick 我要给ESP32点灯`，自动锁定 15~45 分钟敏捷冲刺，免除 10 个 Plan 冗余大纲）。
